...
from tkinter import Tk, _setit, HORIZONTAL, Frame, BOTH, Button, Entry, Label, Text, scrolledtext, END, filedialog, Menu, Radiobutton, IntVar, ttk, OptionMenu, StringVar, Toplevel, Checkbutton, Canvas
import os
import sys
import traceback
...
class APP(Frame):
    ...
    def fill_in_card(self):
        if str(self.index)+"_label.txt" in os.listdir(self.out_path):
            with open(os.path.join(self.out_path, str(self.index)+"_label.txt"), "r") as f:
        ...
        for i, col in enumerate(self.df):
            ...
            for _i in range(0, len(txt), 29):
                ...
                if i <= 10: col1_y_offset += 15
                else: col2_y_offset += 15
    ...
    def create_wigtes(self):
        #Chose in-file
        Button(self.master, text="...", command=self.ask_file, width=3).place(x=15, y=10)
        self.label_in_file_path = Label(text="Choose data source")
        self.label_in_file_path.place(x=55, y=13)

        #Chose out-path
        Button(self.master, text="...", command=self.ask_directory, width=3).place(x=15, y=40)
        ...
        #Label 
        self.current_label = Label(self.master, text="Current label: None", font=("Arial", 11))
        self.current_label.place(x=15, y=503)

        #buttons
        self.btn_prev = Button(self.master, text="Prev", command=self.previous, state="disabled")
        self.btn_prev.place(x=15, y=530, width=370/2-2.5)
        self.btn_next = Button(self.master, text="Next", command=self.next, state="disabled")
        self.btn_next.place(x=15+370/2, y=530, width=370/2-2.5)

        self.buttons = [
            Button(self.master, text="aggro", command=lambda: self.apply_label("aggro"), state="disabled"),
            Button(self.master, text="control", command=lambda: self.apply_label("control"), state="disabled"),
            Button(self.master, text="tempo", command=lambda: self.apply_label("tempo"), state="disabled"),
        ]
    ...
    def check_for_enable(self):
        if all([self.out_path is not None, self.file_in_path is not None]):
            [b.configure(state="normal") for b in self.buttons+[self.btn_next, self.btn_prev]]
    ...
    def ask_directory(self, event=None):
        ...
        filename = "..." + \
            temp_path[-41:] if len(temp_path) >= 39 else temp_path
    ...
    def apply_label(self, label:str):
        self.df.iloc[self.index].to_csv(os.path.join(self.out_path, f"{self.index}.csv"), index=False, header=False)
        with open(os.path.join(self.out_path, str(self.index)+"_label.txt"), "w+") as f:
            f.write(label)
        self.next()
